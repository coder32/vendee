        ...
        <div class="center_column col-xs-12 col-sm-10" id="center_column">
            <div class="profile-board">
                @include('admin.commun.flash-message')
                            ...
                            <h4>Product Detail <small>( {{ Auth::user()->name }} )</small></h4>
                <a href="{{ url('Owner/addProduct') }}" class="btn btn-success-outline" style="float: right">Add Product</a>
                ...
                <tr>
                    ...
                    <?php $i = 1; ?>
                    @foreach ($produ as $comments)
                <tr>
                    <td>{{$i}}</td>
                    <td>{{ $comments->name }}</td>

                    <td>
                        @if($comments->image1 == null)
                            <img src="{{asset('img/avatar0.png')}}" class="img img-thumbnail img-responsive" style="height:50px">
                        @else

                            <img src="{{ URL::asset('../storage/app/public/images') }}/{{$comments->image1}}" class="img img-thumbnail img-responsive" style="height:50px">
                        @endif
                    </td>
                    <td>{{ $comments->created_at }}</td>
                    <td>
                        <a href='{{ url("/Owner/deletePro/{$comments->id}") }}' class="btn btn-danger" onclick="return confirm('Are you sure you want to delete it?')">Delete</a>
                        <a href='{{ url("/Owner/detailProduct/{$comments->id}") }}' class="btn btn-primary">Detail</a>
