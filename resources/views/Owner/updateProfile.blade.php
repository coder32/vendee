    ...
    <div class="container-fluid" id="columns">
        ...
        <div class="center_column col-xs-12 col-sm-10" id="center_column">
            <div class="profile-board">
                ...
                <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" method="post" name="form" enctype="multipart/form-data" action="{{ url('Owner/updateProfiles') }}" >
                    {{csrf_field()}}
                    <input type="hidden" name="uderid" value="{{ Auth::user()->id}}">

                    <div class="profile-data">
                    <div class="col-sm-7 col-xs-12 ">
                        <div class="img"><img src="{{ URL::asset('../storage/app/public/img') }}/{{ auth()->user()->image }}" height="100px"></div>
                        <div class="details_list">
                            <h4>Update Data<small>( {{ Auth::user()->name }} )</small></h4>
                        </div>
                       <input type="text" name="name" required class="form-control" placeholder="Enter Name" value="{{ Auth::user()->name}}"><br>
                        <input type="text" name="email" required class="form-control" placeholder="Enter Slug" value="{{ Auth::user()->email}}"><br>
                        <input type="password" name="password" required class="form-control" placeholder="Enter Slug" value="{{ Auth::user()->password}}"><br>
                        <input type="number" name="phone" required class="form-control" placeholder="Enter Slug" value="{{ Auth::user()->phone}}"><br>
                        <input type="file" name="image" required class="form-control" ><br>
                        <textarea type="text" name="address" required class="form-control" placeholder="Enter Description">{{ Auth::user()->address}}</textarea><br>
