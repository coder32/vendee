            ...
            <!-- Form -->
                <form role="form" method="POST" action="{{ url('password/email') }}">
                    ...
                    <div class="input-with-icon-left {{ $errors->has('email') ? ' has-error' : '' }} has-feedback">
                        ...
                        <input type="Email" id="inp" class="input-text with-border" placeholder="Email Address" name="email" value="{{ old('email') }}">
                    </div>
                    @if ($errors->has('email'))
                    ...
                    <!-- Button -->
                    <button class="button button-sliding-icon pull-right ripple-effect margin-top-10" type="submit" >Submit <i class="icon-material-outline-arrow-right-alt"></i></button>
